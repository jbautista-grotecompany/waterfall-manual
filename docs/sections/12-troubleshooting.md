...
| | Guard open or guard switch not engaging | Close the MAIN GUARD and RETURN #2 guard fully. The switch and actuator must be aligned. Open [MANUAL → SAFETY PLC](11-oi-reference.md#117-manual-screens). If the guard switch still shows red with the guard fully closed, the actuator is misaligned. Adjust the mounting, then press MACHINE ENABLE. | Operator |
| | Guard switch or wiring damaged | If the SAFETY PLC screen shows a guard fault with the guard fully closed and actuator properly seated, inspect the cable from the switch to the panel for damage. If the cables are intact, replace the switch. | Technician |
